...
export type GlassVariant = 'default' | 'accent' | 'danger' | 'ghost';
...
const sizeStyles: Record<GlassSize, React.CSSProperties> = {
  sm:   { padding: '5px 14px', fontSize: '12px', borderRadius: '20px', gap: '5px' },
  md:   { padding: '7px 18px', fontSize: '13px', borderRadius: '24px', gap: '6px' },
  lg:   { padding: '10px 24px', fontSize: '15px', borderRadius: '28px', gap: '8px' },
  icon: { padding: '7px', fontSize: '13px', borderRadius: '50%', gap: '0', width: '34px', height: '34px', justifyContent: 'center' },
};

const variantStyles: Record<GlassVariant, { bg: string; border: string; color: string; shadow: string }> = {
  default: {
  ...
  accent: {
    ...
    shadow: [
      ...
      '0px 1px 5px 0px rgba(0,122,255,0.10)',
      '0px 6px 20px 0px rgba(0,122,255,0.14)',
    ].join(', '),
  },
  danger: {
    ...
    shadow: [
      ...
      '0px 1px 5px 0px rgba(255,59,48,0.08)',
      '0px 4px 12px 0px rgba(255,59,48,0.10)',
    ].join(', '),
  },
  ghost: {
...
const GlassButton = forwardRef<HTMLButtonElement, GlassButtonProps>(
  ({ variant = 'default', size = 'md', isLoading, children, style, disabled, ...props }, ref) => {
        ...
        <svg style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden', pointerEvents: 'none' }} aria-hidden="true">
          <filter id={filterId} primitiveUnits="objectBoundingBox">
            <feImage result="map" width="100%" height="100%" x="0" y="0"
              href={WEBP_DISPLACEMENT_MAP} preserveAspectRatio="none" />
        ...
        >
          ...
          <span
            style={{
              ...
              background: vs.bg,
              backdropFilter: `blur(14px) url(#${filterId}) saturate(180%)`,
          ...
          >
            {isLoading ? (
              <>
                <span className="spinner" style={{ borderColor: 'rgba(0,0,0,0.12)', borderTopColor: vs.color }} />
                {size !== 'icon' && children}
              </>
            ) : children}
          </span>
